...
routes.put("/book/:bookid", async (req, res) => {
    ...
    const updatedBook = await BookModel.findByIdAndUpdate(bookId, updatedBookData, {new: false})
    if(!updatedBook){
...
routes.get("/books/sort", async (req, res) => {
        try {
        const books = await BookModel.find().sort({price: 1}) //1 for ascending order, -1 for descending order
        res.status(200).json({
            status: true,
